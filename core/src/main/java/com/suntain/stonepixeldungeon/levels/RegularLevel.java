...
import com.suntain.stonepixeldungeon.actors.mobs.Thaurissan;
...
import com.suntain.stonepixeldungeon.items.journal.GuidePage;
import com.suntain.stonepixeldungeon.items.keys.GoldenKey;
import com.suntain.stonepixeldungeon.journal.Document;
import com.suntain.stonepixeldungeon.levels.builders.Builder;
...
import com.suntain.stonepixeldungeon.levels.traps.BurningTrap;
import com.suntain.stonepixeldungeon.levels.traps.ChillingTrap;
...
import com.suntain.stonepixeldungeon.levels.traps.WornDartTrap;
...
public abstract class RegularLevel extends Level {
	...
	protected void createMobs() {
		...
		while (mobsToSpawn > 0) {
			...
			Mob mob = null;
			try {
				if(Dungeon.depth < 15 && Dungeon.depth > 10 && !ThaurissanCount) {
					mob = Thaurissan.class.newInstance();
				}
				else mob = createMob();
			} catch (Exception e) {
			...
			if (findMob(mob.pos) == null && passable[mob.pos] && mob.pos != exit) {
				mobsToSpawn--;
				mobs.add(mob);
				if(mob instanceof Thaurissan){
					System.out.println("타우릿산 생성됨");
					ThaurissanCount = true;
				}
