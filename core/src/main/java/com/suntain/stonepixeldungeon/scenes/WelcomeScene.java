...
public class WelcomeScene extends PixelScene {
	...
	public void create() {
		...
		if (StonePixelDungeon.versionCode == previousVersion) {
		...
		StyledButton okay = new StyledButton(Chrome.Type.GREY_BUTTON_TR, Messages.get(this, "continue")){
			...
			protected void onClick() {
				...
				if (previousVersion == 0){
					SPDSettings.version(StonePixelDungeon.versionCode);
					WelcomeScene.this.add(new WndStartGame(1));
				} else {
					updateVersion(previousVersion);
					StonePixelDungeon.switchScene(TitleScene.class);
				}
			}
		};
		...
		if (previousVersion != 0){
			StyledButton changes = new StyledButton(Chrome.Type.GREY_BUTTON_TR, Messages.get(TitleScene.class, "changes")){
				...
				protected void onClick() {
					super.onClick();
					updateVersion(previousVersion);
					StonePixelDungeon.switchScene(ChangesScene.class);
				}
			};
		...
		} else if (previousVersion <= StonePixelDungeon.versionCode) {
	...
	private void updateVersion(int previousVersion){
		...
		if (previousVersion <= StonePixelDungeon.v0_7_0c){
			Badges.loadGlobal();
		...
		if (previousVersion <= StonePixelDungeon.v0_6_5c){
			Journal.loadGlobal();
