...
import com.suntain.stonepixeldungeon.windows.WndError;
...
public class InterlevelScene extends PixelScene {
	...
	public enum Mode {
		MAZE, DESCEND, ASCEND, CONTINUE, RESURRECT, RETURN, FALL, RESET, NONE
	}
	...
	public void update() {
		...
		case STATIC:
			if (error != null) {
				...
				StonePixelDungeon.reportException(
...
//�̷�
	private void maze() throws IOException, InstantiationException, IllegalAccessException {
		Actor.fixTime();
		Dungeon.saveAll();
		Dungeon.level.map[Dungeon.level.specialEntrance] = Terrain.STATUE;
		int d = Dungeon.depth;
		//Dungeon.level.specialEntrance = 0;
		Dungeon.depth = Dungeon.level.specialTarget - 1;
		Level level;
		if(Dungeon.level.specialTarget < 1){
			level = Dungeon.newLevel(d);
		}
		else {
			Dungeon.depth ++;
			level = Dungeon.loadLevel(GamesInProgress.curSlot);
		}
		level.specialTarget = d;
		int pos;
		if(level.specialEntrance == 0){
			pos = level.exit;
			level.specialEntrance = level.exit;
			level.entrance = 0;
			level.exit = 0;
			System.out.println("�δ�����. �칰��ġ : "+level.specialEntrance+", ������ ��ġ : "+level.specialTarget+"�ڽ��� ��ġ : "+pos);
			level.QUEST = Math.abs(Dungeon.depth);
		}
		else{
			pos = level.specialEntrance;
			level.specialEntrance = 0;
			System.out.println("���̿�. �칰��ġ : "+level.specialEntrance+", ������ ��ġ : "+level.specialTarget+"�ڽ��� ��ġ : "+pos);
	...
	private void fall() throws IOException {
		...
		if (Dungeon.depth >= Statistics.deepestFloor) {
			...
			Dungeon.depth++;
			if(Dungeon.depth < 0) Dungeon.depth = Dungeon.level.specialTarget;
			level = Dungeon.loadLevel(GamesInProgress.curSlot);
		}
