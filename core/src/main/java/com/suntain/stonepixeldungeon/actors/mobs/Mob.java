...
import com.suntain.stonepixeldungeon.actors.buffs.Adrenaline;
...
import com.suntain.stonepixeldungeon.actors.buffs.Preparation;
import com.suntain.stonepixeldungeon.actors.buffs.Sleep;
...
import com.suntain.stonepixeldungeon.levels.SewerBossLevel;
...
public abstract class Mob extends Char {
	...
	protected boolean moveSprite( int from, int to ) {
		...
		if (sprite.isVisible() && (Dungeon.level.heroFOV[from] || Dungeon.level.heroFOV[to])) {
			...
			if(rush) {
				...
				if (enemy != null && Dungeon.level.distance(to, enemy.pos) <= 1 && Dungeon.level.distance(from, enemy.pos) > 1) {
					if (canAttack(enemy) && enemy.invisible <= 0 && Dungeon.level.distance(pos, enemy.pos) <= 1) {
	...
	protected boolean getCloser( int target ) {
		...
		if (rooted || target == pos || isWaiting) {
			isWaiting = false;
			return false;
		}
	...
	public void die( Object cause ) {
		...
		if(Dungeon.depth == 5){
			if(this instanceof VoidWalker ||
					this instanceof Imp ||
					this instanceof DoomGuard) ((SewerBossLevel)Dungeon.level).boss.phaze --;
		}
		super.die( cause );
	}
	...
	protected class Wandering implements AiState {
		...
		@Override
		public boolean act( boolean enemyInFOV, boolean justAlerted ) {
			if(Mob.this instanceof DeathKnight) ((DeathKnight)Mob.this).TargetPos = 0;
	...
	protected class Hunting implements AiState {
		...
		public boolean act( boolean enemyInFOV, boolean justAlerted ) {
			...
			if (((me instanceof GnollTrickster &&  enemy != null && enemy.isAlive() && enemy.invisible <= 0) || (!(me instanceof GnollTrickster) && enemyInFOV )) && !isCharmedBy( enemy ) && canAttack( enemy )) {
