...
public class HealthBar extends Component {
	...
	private static final int COLOR_MP	= 0xFF80EEEE;
	...
	public void level( float health, float shield, float mana){
		this.health = health;
		this.shield = shield;
		this.mana = mana;
		layout();
	...
	public void level(Char c){
		...
		float mana = c.MP;
		float max1 = Math.max(health+shield, c.HT);
		float max2 = Math.max(mana, c.MT);
		if(c.MP <= 0) max2 = 0f;
		level(health/max1, (health+shield)/max1, mana / max2);
	}
}
