...
public class WndImp extends Window {
	...
	public WndImp(final Demon demon, final DwarfToken tokens ) {
		...
		RedButton btnReward = new RedButton( Messages.get(this, "reward") ) {
			...
			protected void onClick() {
				takeReward(demon, tokens, Demon.Quest.reward );
			}
		};
	...
	private void takeReward(Demon demon, DwarfToken tokens, Item reward ) {
		...
		if (reward.doPickUp( Dungeon.hero )) {
			...
			Dungeon.level.drop( reward, demon.pos ).sprite.drop();
		}
		
		demon.flee();
		
		Demon.Quest.complete();
	}
}
