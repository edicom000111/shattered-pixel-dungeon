...
public class EmoIcon extends Image {
	...
	public void update() {
		super.update();
		if(!(this instanceof Bub) && !(this instanceof Quest)) {
			if (visible) {
				if (growing) {
	...
	public static class Bub extends EmoIcon {

		public Bub( CharSprite owner ) {

			super(owner);

			copy(Icons.get(Icons.BUB));


			//origin.set( owner.width, owner.height );
			//origin.set( -owner.width, -owner.height );
			origin.set( owner.width, owner.height );
	...
	public static class Quest extends EmoIcon {

		public Quest( CharSprite owner ) {

			super(owner);

			copy(Icons.get(Icons.QUEST));
			...
			x = owner.x + this.width * 0.3f;
			y = owner.y - this.height * 0.7f;
